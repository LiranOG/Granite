...
def run_sim(args):
    exe_path = Path("build/granite_main")
    if sys.platform == "win32":
        exe_path = Path("build/Release/granite_main.exe")
        if not exe_path.exists():
            exe_path = Path("build/granite_main.exe")
            
    if not exe_path.exists():
        print(f"Error: Executable {exe_path} not found. Did you run 'build' first?")
        sys.exit(1)
