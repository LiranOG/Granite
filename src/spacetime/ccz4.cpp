    ...
    for (int k = is; k < ie2; ++k) {
        for (int j = is; j < ie1; ++j) {
            for (int i = is; i < ie0; ++i) {
                ...
                // ──────────────────────────────────────────
                // 8. Advection terms: β^k ∂_k (quantity)
                // ──────────────────────────────────────────
                ...
                for (int ii = 0; ii < 3; ++ii) {
                    for (int jj = ii; jj < 3; ++jj) {
                        int ij = symIdx(ii, jj);
                        Real Lie_gt = 0.0;
                        for (int idx = 0; idx < 3; ++idx) Lie_gt += beta[idx] * d_gt[ij][idx];
                        for (int idx = 0; idx < 3; ++idx) {
                ...
                // Physical Laplacian: D^iD_iα = χ γ̃^{ij}[∂²α − Γ̃∂α] + (3/2χ)γ̃^{ij}∂_iχ∂_jα
                // The conformal connection contribution was missing:
                Real chi_grad_alpha = 0.0;
                for (int a2 = 0; a2 < 3; ++a2)
                    for (int b2 = 0; b2 < 3; ++b2)
                        chi_grad_alpha += gtu[symIdx(a2, b2)] * d_chi[a2] * d_alpha[b2];

                Real physical_lap_alpha = laplacian_alpha / (chi + 1.0e-30)
                                        + 1.5 / (chi + 1.0e-30) * chi_grad_alpha;
                ...
                // 13. RHS: ∂_t Ã_{ij}
                // ──────────────────────────────────────────
                // Placeholder: set to zero (full computation requires TF projection)
                Real rhs_At_raw[6]; 
                Real tr_rhs_At = 0.0;
                ...
                for (int ii = 0; ii < 3; ++ii) {
                    for (int jj = ii; jj < 3; ++jj) {
                        ...
                        Real Lie_At = 0.0;
                        for(int idx=0; idx<3; ++idx) Lie_At += beta[idx] * d_At[ij][idx];
                        for(int idx=0; idx<3; ++idx) {
                ...
                Real rhs_beta[3];
                for (int d = 0; d < 3; ++d) {
                    rhs_beta[d] = 0.75 * Ghat[d] - eta * beta[d];
                }
