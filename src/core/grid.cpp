...
GridBlock::GridBlock(int id,
                    ...
                    int nghost,
                    int num_vars)
    : id_(id), level_(level), nghost_(nghost), num_vars_(num_vars),
      ncells_(ncells), lo_(lo), hi_(hi)
{
    for (int d = 0; d < DIM; ++d) {
...
void GridBlock::packBoundary(int face, std::vector<Real>& buffer) const {
    ...
    switch (face) {
        case 0: is = ng;      ie = 2 * ng; break;  // X-low  interior face
        case 1: is = nx-2*ng; ie = nx-ng;  break;  // X-high interior face
        case 2: js = ng;      je = 2 * ng; break;  // Y-low  interior face
        case 3: js = ny-2*ng; je = ny-ng;  break;  // Y-high interior face
        case 4: ks = ng;      ke = 2 * ng; break;  // Z-low  interior face
        case 5: ks = nz-2*ng; ke = nz-ng;  break;  // Z-high interior face
        default: break;
    }

    buffer.reserve(static_cast<std::size_t>(num_vars_) *
                   static_cast<std::size_t>(ie - is) *
                   static_cast<std::size_t>(je - js) *
                   static_cast<std::size_t>(ke - ks));
...
void GridBlock::unpackBoundary(int face, const std::vector<Real>& buffer) {
    ...
    switch (face) {
        case 0: is = 0;      ie = ng;      break;  // X-low  ghost zone
        case 1: is = nx-ng;  ie = nx;      break;  // X-high ghost zone
        case 2: js = 0;      je = ng;      break;  // Y-low  ghost zone
        case 3: js = ny-ng;  je = ny;      break;  // Y-high ghost zone
        case 4: ks = 0;      ke = ng;      break;  // Z-low  ghost zone
        case 5: ks = nz-ng;  ke = nz;      break;  // Z-high ghost zone
        default: break;
    }

    // Validate buffer size to catch mismatches early
    std::size_t expected = static_cast<std::size_t>(num_vars_) *
                           static_cast<std::size_t>(ie - is) *
                           static_cast<std::size_t>(je - js) *
                           static_cast<std::size_t>(ke - ks);
    if (buffer.size() != expected) {
        ...
        throw std::runtime_error(
            "GridBlock::unpackBoundary: buffer size mismatch. "
