...
Real interpolate(const GridBlock& g, int var, Real px, Real py, Real pz)
{
    ...
    auto d = [&](int ii, int jj, int kk) -> Real {
        return g.data(var, std::max(0, std::min(ii, nx)),
...
Real computeExpansion(const GridBlock& g,
                      ...
                      Real snx, Real sny, Real snz) // coordinate outward normal (unnormalized)
{
    ...
    if (chi < 1.0e-12) return 0.0; // degenerate metric
    ...
    const Real det = gxx*(gyy*gzz - gyz*gyz)
                   - gxy*(gxy*gzz - gyz*gxz)
                   + gxz*(gxy*gyz - gyy*gxz);
    if (std::abs(det) < 1.0e-14) return 0.0;
    ...
    auto scomp = [&](int dir, Real delta) -> Real {
        ...
        if (std::abs(det_l) < 1.0e-14) return 0.0;
        ...
        (void)ihxy_l; (void)ihxz_l; (void)ihyz_l;
        // Return component 'dir' of s^i = γ^{ij} s_j / norm
        if      (dir == 0) return (ihxx_l*sx_l + ihxy_l*sy_l + ihxz_l*sz_l) / nm_l;
        else if (dir == 1) return (ihxy_l*sx_l + ihyy_l*sy_l + ihyz_l*sz_l) / nm_l;
        else               return (ihxz_l*sx_l + ihyz_l*sy_l + ihzz_l*sz_l) / nm_l;
    };
 ...
 */
void buildAngularGrid(int Nth, int Nph,
...
ApparentHorizonFinder::ApparentHorizonFinder(const HorizonParams& params)
    : params_(params)
{}

std::optional<HorizonData>
ApparentHorizonFinder::findHorizon(const GridBlock& spacetime,
                                   std::array<Real, DIM> center) const
{
    ...
    const Real spin_a = (hd.irreducible_mass > 1.0e-12)
                      ? J_z / (hd.irreducible_mass * hd.irreducible_mass)
                      : 0.0;
...
std::vector<HorizonData>
ApparentHorizonFinder::findAllHorizons(const GridBlock& spacetime) const
{
    ...
    for (int k = is; k < ie2; ++k)
        for (int j = is; j < ie1; ++j)
            for (int i = is; i < ie0; ++i) {
                ...
                {
                    candidates.push_back({spacetime.x(0, i),
                                          spacetime.x(1, j),
                                          spacetime.x(2, k)});
                }
            }
    ...
    for (auto& c : candidates) {
        ...
        for (auto& u : unique_candidates) {
            const Real d2 = (c[0]-u[0])*(c[0]-u[0])
                          + (c[1]-u[1])*(c[1]-u[1])
                          + (c[2]-u[2])*(c[2]-u[2]);
            if (d2 < min_sep * min_sep) { dup = true; break; }
        }
        if (!dup) unique_candidates.push_back(c);
    }
...
std::optional<HorizonData>
ApparentHorizonFinder::findCommonHorizon(
    const GridBlock& spacetime,
    const std::vector<HorizonData>& horizons) const
{
    if (horizons.size() < 2) return std::nullopt;
    ...
    if (total_mass < 1.0e-14) return std::nullopt;
    for (int d = 0; d < DIM; ++d)
    ...
    for (const auto& h : horizons) {
        ...
        max_r = std::max(max_r, std::sqrt(dist) + h.circumference_equatorial / (2.0 * constants::PI));
    }
                                        ...
                                        const HorizonData& horizon) const
{
    ...
    if (horizon.circumference_equatorial < 1.0e-12) return 0.0;
    ...
    if (ratio <= ratio_a0) return 0.0;
    if (ratio >= ratio_a1) return 1.0;
