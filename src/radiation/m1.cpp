...
{
    const Real h = g.dx(d);
    auto f = [&](int di, int dj, int dk) -> Real {
        return g.data(var, i+di, j+dj, k+dk);
    };
    if (d == 0)
...
M1Transport::M1Transport(const M1Params& params)
    : params_(params)
{}
                              ...
                              GridBlock& rhs) const
{
    ...
    for (int i = is; i < ie0; ++i) {
        ...
        const Real dPxx_dx = d1(radiation, iER, 0, i, j, k) * (iso + ani*nx*nx) / (Er + 1.0e-30);
        const Real dPxy_dy = d1(radiation, iER, 1, i, j, k) * (ani * nx * ny)  / (Er + 1.0e-30);
        const Real dPxz_dz = d1(radiation, iER, 2, i, j, k) * (ani * nx * nz)  / (Er + 1.0e-30);

        const Real dPyx_dx = d1(radiation, iER, 0, i, j, k) * (ani * ny * nx)  / (Er + 1.0e-30);
        const Real dPyy_dy = d1(radiation, iER, 1, i, j, k) * (iso + ani*ny*ny) / (Er + 1.0e-30);
        const Real dPyz_dz = d1(radiation, iER, 2, i, j, k) * (ani * ny * nz)  / (Er + 1.0e-30);

        const Real dPzx_dx = d1(radiation, iER, 0, i, j, k) * (ani * nz * nx)  / (Er + 1.0e-30);
        const Real dPzy_dy = d1(radiation, iER, 1, i, j, k) * (ani * nz * ny)  / (Er + 1.0e-30);
        const Real dPzz_dz = d1(radiation, iER, 2, i, j, k) * (iso + ani*nz*nz) / (Er + 1.0e-30);
        ...
        (void)Pxx; (void)Pxy; (void)Pxz;
        (void)Pyy; (void)Pyz; (void)Pzz;
        (void)chi;
    }
}
...
// ---------------------------------------------------------------------------
void M1Transport::eddingtonTensor(Real Er, Real Frx, Real Fry, Real Frz,
...
// ---------------------------------------------------------------------------
void M1Transport::computeRadiationForce(
    ...
    std::vector<std::array<Real, SPACETIME_DIM>>& Gmu) const
{
    ...
    for (int i = is; i < ie0; ++i) {
        ...
        if (flat >= static_cast<int>(Gmu.size())) continue;
