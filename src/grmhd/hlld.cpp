...
inline void mhdFlux1D(Real rho, Real vn, Real vt1, Real vt2,
                      Real /*p*/, Real /*eps*/,
                      Real Bn, Real Bt1, Real Bt2,
                      Real D, Real Sn, Real St1, Real St2, Real tau,
                      Real ptot,
...
void GRMHDEvolution::computeHLLDFlux(
...
{
    auto mapVel = [dir](const std::array<Real, NUM_PRIMITIVE_VARS>& p,
                        Real& vn, Real& vt1, Real& vt2) {
        if (dir == 0)      { vn=p[iVX]; vt1=p[iVY]; vt2=p[iVZ]; }
        else if (dir == 1) { vn=p[iVY]; vt1=p[iVZ]; vt2=p[iVX]; }
        else               { vn=p[iVZ]; vt1=p[iVX]; vt2=p[iVY]; }
    };
    auto mapB = [dir](const std::array<Real, NUM_PRIMITIVE_VARS>& p,
                      Real& Bt1, Real& Bt2) {
        if (dir == 0)      { Bt1=p[iPBY]; Bt2=p[iPBZ]; }
        else if (dir == 1) { Bt1=p[iPBZ]; Bt2=p[iPBX]; }
        else               { Bt1=p[iPBX]; Bt2=p[iPBY]; }
    };
    auto mapMom = [dir](const std::array<Real, NUM_HYDRO_VARS>& u,
                        Real& Sn, Real& St1, Real& St2) {
        if (dir == 0)      { Sn=u[iSX]; St1=u[iSY]; St2=u[iSZ]; }
        else if (dir == 1) { Sn=u[iSY]; St1=u[iSZ]; St2=u[iSX]; }
        else               { Sn=u[iSZ]; St1=u[iSX]; St2=u[iSY]; }
    };
    ...
    if (SL >= 0.0) {
        ...
        mhdFlux1D(rhoL, vnL, vt1L, vt2L, pressL, epsL,
                  Bn, Bt1L, Bt2L, uL[iD], SnL, St1L, St2L, uL[iTAU], ptotL, FL);
        if (dir == 0) { flux=FL; }
        else if (dir == 1) {
            flux[iD]=FL[iD]; flux[iSX]=FL[iSZ]; flux[iSY]=FL[iSX];
            flux[iSZ]=FL[iSY]; flux[iTAU]=FL[iTAU];
            flux[iBX]=FL[iBZ]; flux[iBY]=FL[iBX]; flux[iBZ]=FL[iBY];
        } else {
            flux[iD]=FL[iD]; flux[iSX]=FL[iSY]; flux[iSY]=FL[iSZ];
            flux[iSZ]=FL[iSX]; flux[iTAU]=FL[iTAU];
            flux[iBX]=FL[iBY]; flux[iBY]=FL[iBZ]; flux[iBZ]=FL[iBX];
        }
        return;
    }
    if (SR <= 0.0) {
        std::array<Real, NUM_HYDRO_VARS> FR;
        mhdFlux1D(rhoR, vnR, vt1R, vt2R, pressR, epsR,
                  Bn, Bt1R, Bt2R, uR[iD], SnR, St1R, St2R, uR[iTAU], ptotR, FR);
        if (dir == 0) { flux=FR; }
        else if (dir == 1) {
            flux[iD]=FR[iD]; flux[iSX]=FR[iSZ]; flux[iSY]=FR[iSX];
            flux[iSZ]=FR[iSY]; flux[iTAU]=FR[iTAU];
            flux[iBX]=FR[iBZ]; flux[iBY]=FR[iBX]; flux[iBZ]=FR[iBY];
        } else {
            flux[iD]=FR[iD]; flux[iSX]=FR[iSY]; flux[iSY]=FR[iSZ];
            flux[iSZ]=FR[iSX]; flux[iTAU]=FR[iTAU];
            flux[iBX]=FR[iBY]; flux[iBY]=FR[iBZ]; flux[iBZ]=FR[iBX];
        }
        return;
    }
    ...
    if (std::abs(denom_L) > 1e-30 * (std::abs(rhoL*(SL-vnL)*(SL-SM))+1.0)) {
        ...
        vt1L_star = vt1L; vt2L_star = vt2L;
        Bt1L_star = Bt1L; Bt2L_star = Bt2L;
    }
    ...
    if (std::abs(denom_R) > 1e-30 * (std::abs(rhoR*(SR-vnR)*(SR-SM))+1.0)) {
        ...
        vt1R_star = vt1R; vt2R_star = vt2R;
        Bt1R_star = Bt1R; Bt2R_star = Bt2R;
    }

    Real BvL_star = Bn*vnL_star + Bt1L_star*vt1L_star + Bt2L_star*vt2L_star;
    Real BvL     = Bn*vnL + Bt1L*vt1L + Bt2L*vt2L;
    Real tauL_star = ((SL - vnL)*uL[iTAU] - ptotL*vnL + ptot_star*SM
                     + Bn*(BvL - BvL_star)) / (SL - SM + 1e-30);

    Real BvR_star = Bn*vnR_star + Bt1R_star*vt1R_star + Bt2R_star*vt2R_star;
    Real BvR     = Bn*vnR + Bt1R*vt1R + Bt2R*vt2R;
    Real tauR_star = ((SR - vnR)*uR[iTAU] - ptotR*vnR + ptot_star*SM
                     + Bn*(BvR - BvR_star)) / (SR - SM + 1e-30);
    ...
    Real vt1_dstar = (sqrtRhoL_star*vt1L_star + sqrtRhoR_star*vt1R_star
                     + (Bt1R_star - Bt1L_star)*signBn) * inv_sqrtSum;
    Real vt2_dstar = (sqrtRhoL_star*vt2L_star + sqrtRhoR_star*vt2R_star
                     + (Bt2R_star - Bt2L_star)*signBn) * inv_sqrtSum;
    ...
    UstarL[iD]=rhoL_star; UstarL[iSX]=rhoL_star*vnL_star;
    UstarL[iSY]=rhoL_star*vt1L_star; UstarL[iSZ]=rhoL_star*vt2L_star;
    UstarL[iTAU]=tauL_star; UstarL[iBX]=Bn; UstarL[iBY]=Bt1L_star; UstarL[iBZ]=Bt2L_star;

    UstarR[iD]=rhoR_star; UstarR[iSX]=rhoR_star*vnR_star;
    UstarR[iSY]=rhoR_star*vt1R_star; UstarR[iSZ]=rhoR_star*vt2R_star;
    UstarR[iTAU]=tauR_star; UstarR[iBX]=Bn; UstarR[iBY]=Bt1R_star; UstarR[iBZ]=Bt2R_star;

    UdstarL[iD]=rhoL_star; UdstarL[iSX]=rhoL_star*SM;
    UdstarL[iSY]=rhoL_star*vt1_dstar; UdstarL[iSZ]=rhoL_star*vt2_dstar;
    UdstarL[iTAU]=tauL_dstar; UdstarL[iBX]=Bn; UdstarL[iBY]=Bt1_dstar; UdstarL[iBZ]=Bt2_dstar;

    UdstarR[iD]=rhoR_star; UdstarR[iSX]=rhoR_star*SM;
    UdstarR[iSY]=rhoR_star*vt1_dstar; UdstarR[iSZ]=rhoR_star*vt2_dstar;
    UdstarR[iTAU]=tauR_dstar; UdstarR[iBX]=Bn; UdstarR[iBY]=Bt1_dstar; UdstarR[iBZ]=Bt2_dstar;

    std::array<Real, NUM_HYDRO_VARS> F_1d;
    std::array<Real, NUM_HYDRO_VARS> UL_1d, UR_1d;
    UL_1d[iD]=uL[iD]; UL_1d[iSX]=SnL; UL_1d[iSY]=St1L; UL_1d[iSZ]=St2L;
    UL_1d[iTAU]=uL[iTAU]; UL_1d[iBX]=Bn; UL_1d[iBY]=Bt1L; UL_1d[iBZ]=Bt2L;
    UR_1d[iD]=uR[iD]; UR_1d[iSX]=SnR; UR_1d[iSY]=St1R; UR_1d[iSZ]=St2R;
    UR_1d[iTAU]=uR[iTAU]; UR_1d[iBX]=Bn; UR_1d[iBY]=Bt1R; UR_1d[iBZ]=Bt2R;
    ...
    if (dir == 0) {
        flux[iSX]=F_1d[iSX]; flux[iSY]=F_1d[iSY]; flux[iSZ]=F_1d[iSZ];
        flux[iBX]=F_1d[iBX]; flux[iBY]=F_1d[iBY]; flux[iBZ]=F_1d[iBZ];
    } else if (dir == 1) {
        flux[iSX]=F_1d[iSZ]; flux[iSY]=F_1d[iSX]; flux[iSZ]=F_1d[iSY];
        flux[iBX]=F_1d[iBZ]; flux[iBY]=F_1d[iBX]; flux[iBZ]=F_1d[iBY];
    } else {
        flux[iSX]=F_1d[iSY]; flux[iSY]=F_1d[iSZ]; flux[iSZ]=F_1d[iSX];
        flux[iBX]=F_1d[iBY]; flux[iBY]=F_1d[iBZ]; flux[iBZ]=F_1d[iBX];
    }

    if (dir == 0) flux[iBX] = 0.0;
    if (dir == 1) flux[iBY] = 0.0;
    if (dir == 2) flux[iBZ] = 0.0;

    (void)epsL; (void)epsR;
    (void)vnR_star; (void)vnL_star;
}
...
void GRMHDEvolution::computeCTUpdate(
...
Real GRMHDEvolution::divergenceB(const GridBlock& hydro_grid)
{
    ...
    for (int i = is; i < ie0; ++i) {
        ...
        Real dBx_dx = (hydro_grid.data(iBXh, i+1,j,k) - hydro_grid.data(iBXh, i,j,k)) * dxI;
        Real dBy_dy = (hydro_grid.data(iBYh, i,j+1,k) - hydro_grid.data(iBYh, i,j,k)) * dyI;
        Real dBz_dz = (hydro_grid.data(iBZh, i,j,k+1) - hydro_grid.data(iBZh, i,j,k)) * dzI;
