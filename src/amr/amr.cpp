...
{
    // Generic trilinear prolongation interpolation
    if (coarse.getLevel() >= fine.getLevel()) return;
    
    // Stub implementation for now - returning without mutating
    std::cout << "[AMR] Note: Prolongate called but full octree hierarchy is disabled.\n";
}

void AMRHierarchy::restrict_data(const GridBlock& fine, GridBlock& coarse) const
{
    // Volume weighted averaging restriction
    if (fine.getLevel() <= coarse.getLevel()) return;
    
    // Stub implementation for now - returning without mutating
    std::cout << "[AMR] Note: Restrict called but full octree hierarchy is disabled.\n";
}
