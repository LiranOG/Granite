...
{
    // dE/df = (πr²/2) f² |h̃₊ - ih̃×|² (Parseval)
    // Requires FFT of the time series — returned as (freq, dE/df) pairs.
    // Stub: return empty spectrum (no FFT library linked in Granite core).
    (void)l; (void)m; (void)r_ext;
    return {};
}
